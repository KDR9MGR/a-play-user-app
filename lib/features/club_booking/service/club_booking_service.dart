...
class ClubBookingService {
  ...
  Future<BookingModel> createBooking({
    ...
    try {
      ...
      _sendBookingConfirmationEmail(
        ...
        debugPrint('Failed to send club booking confirmation email: $error');
  ...
  Future<void> _sendBookingConfirmationEmail({
    ...
    try {
      ...
      // Check if VIP
      final isVIP = table['location']?.toString().toLowerCase().contains('vip') ?? false;
      ...
      final emailData = {
        ...
        'tableLocation': table['location'] ?? 'Main Floor',
        'totalPrice': totalPrice.toStringAsFixed(2),
        'clubAddress': club['address'] ?? 'See club details',
        'clubPhone': club['phone'] ?? 'Contact via app',
      ...
      await _client.functions.invoke(
        ...
        body: {
          'to': profile['email'],
          'subject': 'VIP Table Confirmed - ${club['name']}',
          'template': 'club-booking',
          'data': emailData,
        },
      );

      debugPrint('Club booking confirmation email sent to ${profile['email']}');
    } catch (e) {
      debugPrint('Error sending club booking confirmation email: $e');
      rethrow;
    }
  }
